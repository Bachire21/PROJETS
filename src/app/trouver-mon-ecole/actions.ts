"use server";

import {
  loadDemandesContent,
  saveDemandesContent,
  appendActivityLog,
} from "@/lib/content-store";
import type { StudentFormData } from "@/data/find-school";
import { nextRequestNumber } from "@/lib/demandes";
import { logStorageError, safeStorageMessage } from "@/lib/storage/errors";

export type SubmitResult = { ok: boolean; message?: string };

export async function submitOrientationRequest(
  data: StudentFormData,
): Promise<SubmitResult> {
  try {
    const content = await loadDemandesContent();
    const now = new Date().toISOString();
    const number = nextRequestNumber(content.requests);

    const request = {
      id: crypto.randomUUID(),
      number,
      status: "nouvelle" as const,
      firstName: data.firstName.trim(),
      lastName: data.lastName.trim(),
      country: data.country,
      whatsapp: data.whatsapp.trim(),
      email: data.email.trim(),
      diploma: data.diploma,
      diplomaYear: data.diplomaYear,
      level: data.educationLevel,
      field: data.desiredField,
      desiredFormation: data.desiredFormation,
      targetLevel: data.targetLevel,
      city: data.desiredCity,
      budget: data.budget,
      housingBudget: data.housingBudget,
      intake: data.intake,
      needs: {
        orientation: data.needs.includes("orientation"),
        admission: data.needs.includes("admission"),
        housing: data.needs.includes("logement"),
        welcome: data.needs.includes("accueil"),
        installation: data.needs.includes("installation"),
        administrative: data.needs.includes("administratif"),
      },
      alreadyAdmitted: data.alreadyAdmitted === "Oui",
      startedSteps: data.startedSteps === "Oui",
      message: data.message.trim(),
      consent: data.consent === "accepted",
      notes: [],
      createdAt: now,
      updatedAt: now,
    };

    content.requests = [request, ...content.requests];
    await saveDemandesContent(content);
    await appendActivityLog(
      `Demande ${number} reçue`,
      `${data.firstName} ${data.lastName}`.trim(),
      "Nouvelle",
    );
    console.log(`[DEBUG] action submitOrientationRequest OK (${number})`);
    return { ok: true };
  } catch (error) {
    logStorageError("submitOrientationRequest", error);
    return {
      ok: false,
      message: safeStorageMessage(
        error,
        "L'envoi de ta demande a échoué. Réessaie dans quelques instants.",
      ),
    };
  }
}